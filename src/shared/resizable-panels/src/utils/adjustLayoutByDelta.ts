...
export function adjustLayoutByDelta({
  ...
  assert(firstPivotIndex != undefined, 'Invalid first pivot index')
  assert(secondPivotIndex != undefined, 'Invalid second pivot index')
...

  {
    ...
    while (true) {
      ...
      const delta = maxSafeSize - previousSize
...

  {
    ...
    while (index >= 0 && index < panelConstraintsArray.length) {
      ...
      if (!fuzzyNumbersEqual(previousSize, safeSize)) {
        deltaApplied += previousSize - safeSize
...

  {
    ...
    if (!fuzzyNumbersEqual(safeSize, unsafeSize)) {
      ...
      while (index >= 0 && index < panelConstraintsArray.length) {
        ...
        if (!fuzzyNumbersEqual(previousSize, safeSize)) {
          deltaRemaining -= safeSize - previousSize
